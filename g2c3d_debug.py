...
def gmaps_landscape(latitude=59.8702089, longitude=29.8649487, zoom=16):
    try:
    ...
    for cnt in contours:
        ...
        if (len(cnt) > 2):
            for point in range(len(cnt)):
                bm_verts.append(bm.verts.new(((cnt[point][0][0] - 320) / 5,
                                              (cnt[point][0][1] - 320) / 5, 0)))
...
def create_road(context, contours, hierarchy):
    ...
    for cnt in contours:
        ...
        if (len(cnt) > 2) and (hierarchy[0][i][3] != -1):
            for point in range(len(cnt)):
                bm2_verts.append(bm2.verts.new(((cnt[point][0][0] - 320) / 5,
                                                (cnt[point][0][1] - 320) / 5, 0)))
    ...
    bpy.ops.object.modifier_add(type='BOOLEAN')
    bpy.context.object.modifiers["Boolean"].operation = "DIFFERENCE"
    bpy.context.object.modifiers["Boolean"].object = ob2
    bpy.ops.object.modifier_apply(apply_as='DATA', modifier="Boolean")
    bpy.ops.object.select_all(action='DESELECT')
...
def create_city(latitude=59.8702089,
                ...
                road = False):
    if plane:
        gmaps_landscape(latitude=59.8702089, longitude=29.8649487, zoom=16)
        create_plane()
    if base:
...
create_city(59.8702089, 29.8649487, 16, 2, 10, True, True, True)
